...
'''
def kmeanstf():
    with open('D:/Python_test/IR/Collection_file32.txt', 'r') as L:
        for lines in tqdm(L):
            lines = lines.strip('\n')
            listOfWords = []
            ...
            for w in listOfWords:
                ...
                if w in stop_words:
                    ...
                    # wordCount[w] = wordCount.get(w, 0.) + 1.0
...
def getkmeans():
    ...
    for word in tqdm(dictionary.keys()):
        j = dictionary[word]
        for i in range(0, K):
            if word in kmeanswordcountperdoc[i]:
                theta[i, j] = kmeanswordcountperdoc[i][word]
    return theta
